...
public class KillEntityProcedure {
	...
	private static void execute(@Nullable Event event, double x, double y, double z, Entity entity, Entity sourceentity) {
		...
		if (sourceentity instanceof Player || sourceentity instanceof ServerPlayer) {
			File = EarnXpGetEntityPathProcedure.execute(entity);
			if (File.exists()) {
				{
					try {
			...
			File = new File((FMLPaths.GAMEDIR.get().toString() + "/EriniumJobs/player_information/" + sourceentity.getUUID().toString()), File.separator + (job_id + ".json"));
			if (File.exists()) {
				{
					try {
						BufferedReader bufferedReader = new BufferedReader(new FileReader(File));
