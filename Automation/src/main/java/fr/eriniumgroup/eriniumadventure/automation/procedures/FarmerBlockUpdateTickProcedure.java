						...
						}
						{
							try {
								...
								while (while_nbr_2 <= 5) {
									...
									}.getItemStack(world, BlockPos.containing(x, y, z), (int) while_nbr_2)).getItem() == ForgeRegistries.ITEMS
											.getValue(new ResourceLocation((JsonObject.get("result").getAsString()).toLowerCase(java.util.Locale.ENGLISH))) || (new Object() {
												public ItemStack getItemStack(LevelAccessor world, BlockPos pos, int slotid) {
													AtomicReference<ItemStack> _retval = new AtomicReference<>(ItemStack.EMPTY);
													BlockEntity _ent = world.getBlockEntity(pos);
													if (_ent != null)
