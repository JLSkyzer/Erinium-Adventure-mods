...
public class Line5PlayerNameProcedure {
	public static String execute(Entity entity) {
		...
		if (!((entity instanceof Player _plrSlotItem && _plrSlotItem.containerMenu instanceof Supplier _splr && _splr.get() instanceof Map _slt ? ((Slot) _slt.get(5)).getItem() : ItemStack.EMPTY).getItem() == Blocks.AIR.asItem())) {
			return entity.getData(EriniumAhModVariables.PLAYER_VARIABLES).theme_text_color + ""
					+ ((entity instanceof Player _plrSlotItem && _plrSlotItem.containerMenu instanceof Supplier _splr && _splr.get() instanceof Map _slt ? ((Slot) _slt.get(5)).getItem() : ItemStack.EMPTY).getOrCreateTag().getString("player"));
		}
		return entity.getData(EriniumAhModVariables.PLAYER_VARIABLES).theme_text_color;
	}
}
