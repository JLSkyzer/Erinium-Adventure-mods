...
public class SafezoneClaimProcedure {
			...
			if (IsWildernessProcedure.execute(world, entity)) {
				File = GetChunkPathProcedure.execute(world, entity);
				{
					try {
						...
						JsonObject.addProperty("captured_by", "Safezone");
						{
				...
				if (entity instanceof Player _player && !_player.level().isClientSide())
					_player.displayClientMessage(Component.literal("\u00A74Already Safezone or bypass claim not enabled !"), false);
			}
		}
	}
}
