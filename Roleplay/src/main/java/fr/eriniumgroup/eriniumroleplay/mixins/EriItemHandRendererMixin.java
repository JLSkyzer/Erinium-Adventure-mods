...
public abstract class EriItemHandRendererMixin<T extends Player, M extends EntityModel<T> & ArmedModel & HeadedModel> extends ItemInHandLayer<T, M> {
    private final ItemInHandRenderer itemInHandRenderer;
    private static final float X_ROT_MIN = (float) (-Math.PI / 6);
    private static final float X_ROT_MAX = (float) (Math.PI / 2);

    public EriItemHandRendererMixin(RenderLayerParent<T, M> p_234866_, ItemInHandRenderer p_234867_) {
    ...
    protected void renderArmWithItem(
            LivingEntity p_270884_,
            ItemStack p_270379_,
            ItemDisplayContext p_270607_,
            HumanoidArm p_270324_,
            PoseStack p_270124_,
            MultiBufferSource p_270414_,
            int p_270295_
    ) {
        if (!p_270884_.getData(EriniumroleplayModVariables.PLAYER_VARIABLES).vanish_mode){
